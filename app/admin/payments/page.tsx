...
export default function PaymentsPage() {
  ...
  const handleCreatePayment = async () => {
    ...
    try {
      // Tìm order để lấy thông tin bàn
      const selectedOrder = orders?.find(o => o.id === formData.orderId);

      // Tự động end table nếu đang PLAYING
      // if (selectedOrder?.table?.status === "PLAYING") {
      // }
      if (selectedOrder?.tableId) await tablesApi.end(selectedOrder?.tableId);
      ...
      toast({
        title: "Success",
        description: "Payment completed & table ended",
      });
  ...
  const getMethodBadge = (method: PaymentMethod) => {
    ...
    return (
      <Badge variant={config[method].variant}>{config[method].label}</Badge>
    );
  };
                  ...
                  <SelectContent>
                    {(orders || []).map(order => (
                      <SelectItem key={order.id} value={order.id}>
                        Order #{order.id.slice(0, 8)} - Table{" "}
                        {order.table?.code} - {order.total.toLocaleString()}đ
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
        ...
        <Card className="dark:border-slate-800 dark:bg-slate-900">
          <CardContent className="pt-6">
            <div className="text-3xl font-bold text-purple-500 mb-1">
              ...
              {(payments || []).filter(p => p.method === "MOMO").length}
            </div>
            <div className="text-sm text-slate-600 dark:text-slate-400">
              Cash/MoMo
            </div>
